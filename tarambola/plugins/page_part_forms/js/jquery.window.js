...
//			bookmarkable: true,           // [boolean:true] to control window with remote url could be bookmarked
		...
		function initialize(instance) {
			...
			// add bookmark button
			/*
			if( options.bookmarkable && options.url != null && jQuery.trim(options.url) != "" ) {
		...
		function doBookmark(title, url) {
			if ( jQuery.browser.mozilla && window.sidebar ) { // Mozilla Firefox Bookmark
				window.sidebar.addPanel(title, url, "");
			} else if( jQuery.browser.msie && window.external ) { // IE Favorite
			...
			} else if(jQuery.browser.safari || ua.indexOf("safari") >= 0 ) { // Safari
				alert("Sorry! Safari doesn't support bookmark function currently.");
				//alert("Press [Ctrl + D] to bookmark in Safari");
			} else if(jQuery.browser.opera || ua.indexOf("opera") >= 0 ) { // Opera Hotlist
